...
# 均值滤波
def AverageFilter(original, kernel=kernel_5x5):
    '''
    :param image: 图片矩阵
    :param kernel: 滤波窗口
    :return:均值滤波后的矩阵
...
# 高斯滤波
def GaussianFilter(Image,sigma=2):
    '''
    :param sigma: σ标准差
    :return: 高斯滤波器的模板
    '''
...
def FFT_2d(Img):
    pic = np.fft.fft2(Img)
    return pic
LPFGPU = mod5.get_function("LPFGPU");

def D(u,v,window_size):
    (P,Q) = window_size
...
def LPF_cuda(fft,D_0 = 20):
    ...
    LPFGPU(destImage_gpu, D_0 ,DATA_W, DATA_H, block=(imageHeight, 1, 1), grid=(1, imageWidth))
    cuda.memcpy_dtoh(destImage, destImage_gpu)
